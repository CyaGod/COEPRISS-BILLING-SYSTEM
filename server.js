...
app.post('/api/facturas/limpiar-falsas', autenticarToken, async (req, res) => {
    try {
        ...
        if (!password || String(password).trim() === '') {
            return res.status(400).json({ success: false, error: 'Se requiere la clave maestra de seguridad para autorizar la purga de datos.' });
        }

        const adminMasterPass = (process.env.ADMIN_CLEAN_PASSWORD || 'COEPRISS2026PROD').trim();

        if (password !== adminMasterPass) {
            await registrarBitacora(req.user?.id || 1, 'LIMPIEZA_RECHAZADA', 'Intento de purga de base de datos con clave incorrecta', req.ip, 'DENEGADO');
            return res.status(403).json({ success: false, error: 'Clave maestra de seguridad incorrecta. Acción no autorizada.' });
        }

        const resultado = await vaciarFacturasYExpedientesPrueba();
        await registrarBitacora(req.user?.id || 1, 'LIMPIEZA_TOTAL_PRODUCCION', 'Base de datos vaciada y reinicializada para producción con clave maestra de seguridad', req.ip, 'EXITOSO');
    ...
    try {
        ...
        const rows = expedientes.map(e => {
            ...
            return {
                ...
                'Exportación': e.cfdiExportacion || '01',
                'Concepto / Descripción': e.cfdiConcepto || '',
                'Clave Producto/Servicio SAT': e.cfdiClaveProdServ || '90101501',
                'Clave Unidad': e.cfdiClaveUnidad || 'ACT',
                'Unidad': e.cfdiUnidad || 'Actividad',
                'Cantidad': Number(parseFloat(e.cfdiCantidad || 1).toFixed(3)),
                'Objeto de Impuesto': e.cfdiObjetoImp || '02',
                'Tipo de Impuesto': e.cfdiTipoImpuesto || 'IVA',
                'Tasa de Impuesto': `${Number(parseFloat(e.cfdiTasaImpuesto || 0.16) * 100).toFixed(2)}%`,
                'Es Retención': e.cfdiEsRetencion ? 'Sí' : 'No',
        ...
        if (!expediente && req.body.expediente) {
            ...
            expediente = await prisma.expediente.upsert({
                ...
                update: {
                    ...
                    cfdiMoneda:       d.moneda        || d.cfdiMoneda        || 'MXN',
                    cfdiExportacion:  d.exportacion   || d.cfdiExportacion   || '01',
                    cfdiClaveProdServ: d.claveProdServ || d.cfdiClaveProdServ || '90101501',
                    cfdiClaveUnidad:  d.claveUnidad   || d.cfdiClaveUnidad   || 'ACT',
                    cfdiUnidad:       d.unidad        || d.cfdiUnidad        || 'Actividad',
                    cfdiCantidad:     parseFloat(d.cantidad ?? d.cfdiCantidad ?? 1),
                    cfdiObjetoImp:    d.objetoImp     || d.cfdiObjetoImp     || '02',
                    cfdiTipoImpuesto: d.tipoImpuesto  || d.cfdiTipoImpuesto  || 'IVA',
                    cfdiTasaImpuesto: parseFloat(d.tasaImpuesto ?? d.cfdiTasaImpuesto ?? 0.16),
                    cfdiEsRetencion:  d.esRetencion   ?? d.cfdiEsRetencion   ?? false,
                },
                create: {
                    folio: expedienteId,
                    usuarioId: req.user.id,
